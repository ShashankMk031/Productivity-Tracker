...
def get_report_history(db: sqlite3.Connection):
    rows = db.execute("SELECT id, type, generated_at, summary, period_start, period_end, ai_provider, ai_model FROM reports ORDER BY generated_at DESC").fetchall()
...
def generate_and_save_report(db: sqlite3.Connection, report_type: str, report_date = None, replace_report_id: int | None = None):
    ...
    ai_reflection, ai_provider, ai_model = ai_service.generate_reflection(ai_prompt, context=ai_context)
    ...
    md_content += f"*AI Provider: {ai_provider} | Model: {ai_model}*\n"
    ...
    if replace_report_id is not None:
        db.execute(
            "UPDATE reports SET type = ?, generated_at = ?, markdown_path = ?, summary = ?, period_start = ?, period_end = ?, ai_provider = ?, ai_model = ? WHERE id = ?",
            (report_type, generated_at, str(filepath), summary, period_start, period_end, ai_provider, ai_model, replace_report_id)
        )
        report_id = replace_report_id
    else:
        cur = db.execute(
            "INSERT INTO reports (type, generated_at, markdown_path, summary, period_start, period_end, ai_provider, ai_model) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (report_type, generated_at, str(filepath), summary, period_start, period_end, ai_provider, ai_model)
        )
